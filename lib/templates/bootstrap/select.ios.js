...
import { Animated, View, TouchableOpacity, Text, Picker, PanResponder } from "react-native";
...
class CollapsiblePickerIOS extends React.Component {
    constructor(props) {
        ...
        this.onResponderMove = this._onResponderMove.bind(this);
    ...
    _onValueChange(val) {
        const { onChange, value, collapseDelay } = this.props.locals;
        const delay = collapseDelay ? collapseDelay : 0;

        if (val !== value) {
            this.collapseTimeout = setTimeout(this.togglePicker, delay);
            onChange(val);
        }
    }
